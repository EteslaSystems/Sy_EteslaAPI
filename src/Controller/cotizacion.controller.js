...
module.exports.getSecondStepCotizacion = async function(data){
    let Result = {};
...
async function busquedaInteligente(data){ /* [ Combinaciones ] */
    ...
    try{
        ...
        //Datos cliente
        let Cliente = await ClienteController.consultarId({ idPersona: data.idCliente });
        Cliente = Cliente.message; 
        Cliente = Cliente[0];

        //Datos vendedor
        let Vendedor = await UsuarioController.consultarId({ idPersona: data.idUsuario });
...
//@static
async function filtrarEquiposSelectos(MatEquipoSelect, _equipos, combinacionName, recursive){
...
/*#region CRUD*/
module.exports.insertar = async function(objPropuesta){
    try{
...
//@
module.exports.consulta = async function(data){
